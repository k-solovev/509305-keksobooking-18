...
(function () {
  ...
  var createAdCard = function (obj) {
    ...
    adElement.querySelector(
        '.popup__type'
    ).textContent = window.data.getTranslateType(obj.offer.type);
    ...
    adElement.querySelector(
        '.popup__features'
    ).innerHTML = window.data.getFeatureIcon(obj.offer.features);
    adElement.querySelector('.popup__description').textContent =
      obj.offer.description;
    adElement.querySelector('.popup__photos').innerHTML = window.data.getImages(
        obj.offer.photos
    );
